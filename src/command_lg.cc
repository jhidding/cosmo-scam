...
void command_lg(int argc_, char **argv_)
{
	Argv argv = read_arguments(argc_, argv_,
		...
		Option({0, "pc", "perspective-camera", "false",
		  "use a camera with perspective."}),
		...
		Option({Option::VALUED | Option::CHECK, "w", "wall-lim", "10.0",
			"lower limit of wall density to show (squared)."}),
		Option({Option::VALUED | Option::CHECK, "wga", "wall-gradient-start", "5.0",
			"lower density limit of gradient."}),
		Option({Option::VALUED | Option::CHECK, "wgb", "wall-gradient-end", "5.0",
			"upper density limit of gradient."}));
	...
	for (unsigned i = 0; i < slices.size(); ++i)
	{
        ...
        bool pc = argv.get<bool>("perspective-camera");
        if (pc)
        {
//            cam_pos = Point( -1.642 * L, 2.687 * L, -0.711 * L);
            cam_pos = Point( -1.642 * L, 1.711 * L, -1.687 * L);
            cam_tgt = Point(L/2, L/2, L/2);
            cam_shub = Vector(0.689, 0.566, -0.452);
        }
    
		auto C = make_ptr<Camera>(cam_pos, cam_tgt, cam_shub,
			(pc ? weak_perspective_projection(2.0*L) : parallel_projection));
